...
int _FFT_GetAmplitude(int i)
{
	if(i == 0)
	{
		return (uint16_t)((sqrt(sq(vReal[0]) + sq(vImag[0])) / ADC_SAMPLE_COUNT));	
	}
	else
	{
		return (uint16_t)(sqrt(sq(vReal[i]) + sq(vImag[i])) * 2 / ADC_SAMPLE_COUNT);
	}
}
...
i2s_config_t adcI2SConfig = {
    ...
    .communication_format = I2S_COMM_FORMAT_I2S_LSB,	// 设置交流格式
   ...
   // set up the adc sample writer task
  	TaskHandle_t adcWriterTaskHandle;
   	xTaskCreatePinnedToCore(FFT_adcWriterTask, "ADC Writer Task", 4096, &g_adcSampler, 1, &adcWriterTaskHandle, 1);
  	g_adcSampler.start(I2S_NUM_0, adcI2SConfig, ADC_SAMPLE_COUNT * 2, adcWriterTaskHandle);  
}

bool FFT_GetDataFlag()
{
  return g_fftDataIsOk;
}
